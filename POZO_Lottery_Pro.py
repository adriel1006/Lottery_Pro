...
class LottoApp(QMainWindow):
    ...
    def fetch_latest_results(self):
        ...
        if response.status_code == 200:
            ...
            results_container = soup.find('div', class_='column is-12 lg-center-flex')
            if results_container:
                numbers = [li.text.strip() for li in results_container.find_all('li', class_='lg-number')]
                return numbers
            else:
    ...
    def setup_update_tab(self):
        ...
        self.latest_results_text_edit = QTextEdit()
        self.latest_results_text_edit.setReadOnly(True)
        layout.addWidget(self.latest_results_text_edit)

        self.countdown_display = QLabel("Time until next draw: ")
    ...
    def update_countdown_display(self):
        now = datetime.now()
        next_draw = now + timedelta(days=(7 - now.weekday()) % 7)  # Next Monday
        next_draw = next_draw.replace(hour=11, minute=0, second=0, microsecond=0)
        if now.weekday() == 0 and now.hour >= 11:  # If today is Monday and it's past 11 AM
            next_draw += timedelta(days=7)
        time_left = next_draw - now
        self.countdown_display.setText(f"Time until next draw: {time_left}")

    def display_latest_results(self):
        results = self.fetch_latest_results()
        if isinstance(results, list):
            formatted_results = ', '.join(map(str, results))
            self.latest_results_text_edit.setText(formatted_results)
        else:
            error_message = "No results to display or error fetching results."
            self.latest_results_text_edit.setText(error_message)
    ...
    # Buttons to switch graphs
        ...
        self.sum_totals_button = QPushButton("Show Sum Totals")
        self.sum_totals_button.clicked.connect(self.display_sum_totals)
        layout.addWidget(self.sum_totals_button)
    ...
    def display_sum_totals(self):
        if not past_winning_numbers:
            self.results_display.setText("No winning numbers data available.")
            return

        sum_totals = [sum(numbers) for numbers in past_winning_numbers]
        sessions = [1065 + i for i in range(len(past_winning_numbers))]
    
        self.figure.clear()  # Clear the existing plot
        ax = self.figure.add_subplot(111)
        bars = ax.bar(sessions, sum_totals, color='skyblue', edgecolor='black')  # Use bar plot

        ax.set_title('Sum Totals of Winning Numbers')
        ax.set_xlabel('Session Number')
        ax.set_ylabel('Sum Total')

        # Annotate each bar with the sum total
        for bar in bars:
            yval = bar.get_height()
            ax.text(bar.get_x() + bar.get_width()/2, yval, int(yval), 
                    verticalalignment='bottom',  # Vertical alignment can be 'bottom', 'center', or 'top'
                    horizontalalignment='center',  # Horizontal alignment can be 'left', 'right', or 'center'
                    color='black', fontsize=8)

        ax.set_xticks(sessions)  # Set x-ticks to session numbers
        ax.set_xticklabels([str(session) for session in sessions], rotation=45)  # Rotate labels for better visibility
